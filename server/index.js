...
// Extended detail data keyed by ticker
const projectDetails = {
  ...
  AZTEC: {
    ...
    founded: "2022",
    contracts: [
      { label: "Token", address: "0xA3c1...8f2B", explorer: "etherscan" },
    ],
  ...
  UP: {
    ...
    founded: "2023",
    contracts: [
      { label: "Token", address: "0xB5f2...2c9A", explorer: "basescan" },
    ],
  ...
  FO: {
    ...
    founded: "2024",
    contracts: [
      { label: "Token", address: "0xFo3g...1b4C", explorer: "etherscan" },
    ],
    explorers: ["Etherscan"],
  ...
  AUTISM: {
    ...
    founded: "2024",
    contracts: [
      { label: "Token", address: "Aut1sm...3Xpq", explorer: "solscan" },
    ],
    explorers: ["Solscan", "Solana Explorer"],
    wallets: ["Phantom", "Solflare", "Backpack"],
    trustAiSummary:
...
app.get("/api/projects/most-talked", (req, res) => {
  const mostTalked = projects.filter((p) => p.category === "mostTalked");
  res.json({ success: true, data: mostTalked });
});

app.get("/api/projects/launching-soon", (req, res) => {
  const launching = projects.filter((p) => p.category === "launchingSoon");
  res.json({ success: true, data: launching });
});

app.get("/api/projects/:ticker", (req, res) => {
  const ticker = req.params.ticker.toUpperCase();
  const base = projects.find((p) => p.ticker === ticker);
  if (!base) return res.status(404).json({ success: false, error: "Project not found" });
  const detail = projectDetails[ticker] || {};
  res.json({ success: true, data: { ...base, ...detail } });
});

app.listen(PORT, () => {
  console.log(`Trust.net API running on http://localhost:${PORT}`);
});
